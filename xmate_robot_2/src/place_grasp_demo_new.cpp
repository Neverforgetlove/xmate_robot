...
{ 
    ...
    //抓取类型对应的二维码号, 螺丝螺母钣金件
    int grasp_ar_id[2] = {1,2}; 

    //当前抓取目标id
    int current_grasp_id = -1;
    ...
    try{
        ...
        {   
   		...
   		LS_Grasp = true;
                ...
                ww = Robot_Interface.Ar_Pose[grasp_ar_id[1]][3];
                wx = Robot_Interface.Ar_Pose[grasp_ar_id[1]][4];
                wy = Robot_Interface.Ar_Pose[grasp_ar_id[1]][5];
                wz = Robot_Interface.Ar_Pose[grasp_ar_id[1]][6];
            ...
            if(Robot_Interface.Ar_Pose[grasp_ar_id[0]][0]!=0.0 && LM_Grasp == false)
            {
                ...
                ww1 = Robot_Interface.Ar_Pose[grasp_ar_id[0]][3];
                wx1 = Robot_Interface.Ar_Pose[grasp_ar_id[0]][4];
                wy1 = Robot_Interface.Ar_Pose[grasp_ar_id[0]][5];
                wz1 = Robot_Interface.Ar_Pose[grasp_ar_id[0]][6];
            ...
            if(Robot_Interface.Ur_Pose[0][0]!=0.0 && BJJ_Grasp == false)
            {
                ...
                //四元数转欧拉角
                Eigen::Vector3d eulerAngle2=quaternion_1.matrix().eulerAngles(2,1,0);
                //弧度转角度
        ...
        {   
            ...
            if(Robot_Interface.Ar_Pose[grasp_ar_id[1]][0]!=0.0 && LS_Grasp == false)
            {
                ...
                Eigen::Quaterniond quaternion(ww,wx,wy,wz);
                //四元数转欧拉角
                Eigen::Vector3d eulerAngle3=quaternion.matrix().eulerAngles(2,1,0);
                //弧度转角度
                robot_angle = (eulerAngle3(0) * 180) / PI;
                if(robot_angle > 90.0){
                        robot_angle = 180.0 - robot_angle;	
                }
                if(robot_angle > 90){
                    //  机械臂末端旋转
                    Robot_Interface.Robot_MoveR(-PI/2 + eulerAngle3(0), robot);
                }else{
                    //  机械臂末端旋转
                    Robot_Interface.Robot_MoveR(eulerAngle3(0) - PI/2, robot);
                }
                ...
                Robot_Interface.clean_ar_data();
                //关夹爪
                bool close_state3 = Robot_Interface.Robot_Grasp_Control(0,robot);
                if(close_state3){
                    //往上
                    Robot_Interface.Robot_MoveL(0.0,0.0,graspid2_z,robot);
                    Robot_Interface.Robot_MoveJ(Robot_Interface.grasp_identify_pose,robot);
                ...
                bool open_state3 = Robot_Interface.Robot_Grasp_Control(1,robot);
                //往上
                Robot_Interface.Robot_MoveL(0.0,0.0,graspid2_z,robot);
                if(open_state3){
                    //初始位姿
            ...
            {	
                ...
                Eigen::Quaterniond quaternion1(ww1,wx1,wy1,wz1);
                //四元数转欧拉角
                Eigen::Vector3d eulerAngle4=quaternion1.matrix().eulerAngles(2,1,0);
                //弧度转角度
                robot_angle = (eulerAngle4(0) * 180) / PI;
                if(robot_angle > 90.0){
                        robot_angle = 180.0 - robot_angle;	
                }
                if(robot_angle > 90){
                    //  机械臂末端旋转
                    Robot_Interface.Robot_MoveR(-PI/2 + eulerAngle4(0), robot);
                }else{
                    //  机械臂末端旋转
                    Robot_Interface.Robot_MoveR(eulerAngle4(0) - PI/2, robot);
                }
                ...
                Robot_Interface.clean_ar_data();
                //关夹爪
                bool close_state4 = Robot_Interface.Robot_Grasp_Control(0,robot);
                if(close_state4){
                    //往上
                    Robot_Interface.Robot_MoveL(0.0,0.0,graspid1_z,robot);
                    Robot_Interface.Robot_MoveJ(Robot_Interface.grasp_identify_pose,robot);
                ...
                bool open_state4 = Robot_Interface.Robot_Grasp_Control(1,robot);
                //往上
                Robot_Interface.Robot_MoveL(0.0,0.0,graspid1_z,robot);
                if(open_state4){
                    //初始位姿
            ...
            {	
                ...
                Eigen::Quaterniond quaternion_1(ww,wx,wy,wz);
                //四元数转欧拉角
                Eigen::Vector3d eulerAngle6=quaternion_1.matrix().eulerAngles(2,1,0);
                //弧度转角度
                robot_angle = (eulerAngle6(0) * 180) / PI;
                if(robot_angle > 90.0){
                        robot_angle = 180.0 - robot_angle;	
                }
                if(robot_angle > 90){
                    //  机械臂末端旋转
                    Robot_Interface.Robot_MoveR(-PI/2 + eulerAngle6(0), robot);
                }else{
                    //  机械臂末端旋转
                    Robot_Interface.Robot_MoveR(eulerAngle6(0) - PI/2, robot);
                }
                ...
                bool close_state5 = Robot_Interface.Robot_Grasp_Control(0,robot);
                if(close_state5){
                    //往上
                    Robot_Interface.Robot_MoveL(0.0,0.0,bjj_place_z,robot);
                    Robot_Interface.Robot_MoveJ(Robot_Interface.grasp_identify_pose,robot);
                ...
                bool open_state5 = Robot_Interface.Robot_Grasp_Control(1,robot);
                //往上
                Robot_Interface.Robot_MoveL(0.0,0.0,bjj_place_z,robot);
                if(open_state5){
                    //初始位姿
        ...
        //四元数转欧拉角
        Eigen::Vector3d eulerAngle5 =quaternion_2.matrix().eulerAngles(2,1,0);
        //弧度转角度
        robot_angle = (eulerAngle5(0) * 180) / PI;
        if(robot_angle > 90.0){
	            robot_angle = 180.0 - robot_angle;	
	    }
        if(robot_angle > 90){
            //  机械臂末端旋转
            Robot_Interface.Robot_MoveR(-PI/2 + eulerAngle5(0), robot);
            robot_angle = -PI/2 + eulerAngle5(0);
        }else{
            //  机械臂末端旋转
            Robot_Interface.Robot_MoveR(eulerAngle5(0) - PI/2, robot);
            robot_angle = eulerAngle5(0) - PI/2;
        }
        ...
        //关夹爪
	    bool close_state6 = Robot_Interface.Robot_Grasp_Control(0,robot);
        if(close_state6){
            //往上
            Robot_Interface.Robot_MoveL(0.0, 0.0, placeid2_z, robot);
            //物料盘上方
        ...
        Robot_Interface.Robot_MoveL(0.0,0.0,-(sz3-place_z),robot);
	    bool open_state6 = Robot_Interface.Robot_Grasp_Control(1,robot);
        if(open_state6){
            Robot_Interface.Robot_MoveL(0.0,0.0,sz3-place_z,robot);
            //放置识别位姿
            Robot_Interface.Robot_MoveJ(Robot_Interface.place_identify_pose,robot);
...
/*-----------------------------id1 螺丝-----------------------------*/
        ...
        //四元数转欧拉角
        Eigen::Vector3d eulerAngle6=quaternion_3.matrix().eulerAngles(2,1,0);
        //弧度转角度
        robot_angle = (eulerAngle6(0) * 180) / PI;
        if(robot_angle > 90.0){
	            robot_angle = 180.0 - robot_angle;	
        ...
        if(robot_angle > 90){
            //  机械臂末端旋转
            Robot_Interface.Robot_MoveR(-PI/2 + eulerAngle6(0), robot);
            robot_angle = -PI/2 + eulerAngle6(0);
        }else{
            //  机械臂末端旋转
            Robot_Interface.Robot_MoveR(eulerAngle6(0) - PI/2, robot);
            robot_angle = eulerAngle6(0) - PI/2;
        }
        ...
        //关夹爪
	    bool close_state7 = Robot_Interface.Robot_Grasp_Control(0,robot);
        if(close_state7){
            //往上
            Robot_Interface.Robot_MoveL(0.0, 0.0, placeid1_z, robot);
            //物料盘上方
        ...
        Robot_Interface.Robot_MoveL(sx3-place_x,-sy3+place_y,0.0,robot);
        Robot_Interface.Robot_MoveL(0.0,0.0,-(sz3-place_z),robot);
	    bool open_state7 = Robot_Interface.Robot_Grasp_Control(1,robot);
        if(open_state7){
            Robot_Interface.Robot_MoveL(0.0,0.0,sz3-place_z,robot);
            //放置识别位姿
            Robot_Interface.Robot_MoveJ(Robot_Interface.place_identify_pose,robot);
...
/*-----------------------------id3, 钣金件-----------------------------*/
        ...
        //四元数转欧拉角
        Eigen::Vector3d eulerAngle7=quaternion2.matrix().eulerAngles(2,1,0);
        //弧度转角度
        robot_angle = (eulerAngle7(0) * 180) / PI;
        if(robot_angle > 90.0){
	            robot_angle = 180.0 - robot_angle;	
	    }
        if(robot_angle > 90){
            //  机械臂末端旋转
            Robot_Interface.Robot_MoveR(-PI/2 + eulerAngle7(0), robot);
            robot_angle = -PI/2 + eulerAngle7(0);
        }else{
            //  机械臂末端旋转
        ...
        //关夹爪
	    bool close_state8 = Robot_Interface.Robot_Grasp_Control(0,robot);
        if(close_state8){
            //往上
            Robot_Interface.Robot_MoveL(0.0, 0.0, zpt_bjj_grasp_z, robot);
            //物料盘上方
        ...
        Robot_Interface.Robot_MoveL(0.0,0.0,-(sz3-zpt_bjj_place_z),robot);
	    bool open_state8 = Robot_Interface.Robot_Grasp_Control(1,robot);
        if(open_state8){
            Robot_Interface.Robot_MoveL(0.0,0.0,sz3-zpt_bjj_place_z,robot);
            //放置识别位姿
            Robot_Interface.Robot_MoveJ(Robot_Interface.place_identify_pose,robot);
	...
	/*-----初步校准-----*/
        ...
        bool close_state9 = Robot_Interface.Robot_Grasp_Control(0,robot);
        if(close_state9){
            //往上
            Robot_Interface.Robot_MoveL(0.0, 0.0, sz3-cp_bjj_place_z, robot);
            //物料盘上方
        ...
        //开夹爪
	    bool open_state9 = Robot_Interface.Robot_Grasp_Control(1,robot);
        if(open_state9){
            //往上
            Robot_Interface.Robot_MoveL(0.0, 0.0, cp_bjj_grasp_z, robot);
            //物料盘上方
